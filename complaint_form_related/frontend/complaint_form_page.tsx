...
export default function Home() {
  ...
  const handleSubmitComplaint = async (e: React.FormEvent) => {
    ...
    const categoryLabel = caseTypeMapping[selectedCategory] || selectedCategory
    const evidenceText = [
      formData.evidence.trim(),
      `Uploaded files: ${uploadedImages.map((f) => f.name).join(', ')}`
    ].filter(Boolean).join(' | ')

    setIsSubmittingComplaint(true)
    try {
      const response = await fetch(`${API_BASE_URL}/create-complaint`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          title: `${categoryLabel} Case`,
          description: formData.description.trim(),
          crime_type: categoryLabel,
          jurisdiction: formData.location.trim() || 'Not specified',
          is_anonymous: false,
      ...
      alert(`Complaint filed successfully for ${categoryLabel}`)
